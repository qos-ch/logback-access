...
public class EnsureLineSeparation implements PostCompileProcessor<IAccessEvent> {
    ...
    public void process(Context context, Converter<IAccessEvent> head) {
        ...
        Converter<IAccessEvent> newLineConverter = new ch.qos.logback.access.common.pattern.LineSeparatorConverter();
        if (!(tail instanceof ch.qos.logback.access.common.pattern.LineSeparatorConverter)) {
